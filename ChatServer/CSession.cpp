...
void CSession::Send(std::string msg, short msgid) {
	...
	auto& msgnode = _send_que.front();
	std::cout << "msgnode_data: " << msgnode->_data << " msgnode_total_len: " << msgnode->_total_len <<
		" msgnode_curlen" << msgnode->_cur_len << std::endl;
	boost::asio::async_write(_socket, boost::asio::buffer(msgnode->_data, msgnode->_total_len),
		std::bind(&CSession::HandleWrite, this, std::placeholders::_1, SharedSelf()));
}
...
void CSession::HandleWrite(const boost::system::error_code& error, std::shared_ptr<CSession> shared_self)
{
	try {
		if (!error) {
			...
			if (!_send_que.empty()) {
				auto msgnode = _send_que.front();
				boost::asio::async_write(_socket, boost::asio::buffer(msgnode->_data, msgnode->_cur_len),
					std::bind(&CSession::HandleWrite, this, std::placeholders::_1, SharedSelf()));
			}
		}
		else {
			std::cout << "HandleWrite faild, error is: " << error.what() << std::endl;
			Close();
			_server->ClearSession(_session_id);
		}
		...
		std::cout << " handlewrite exception is: " << e.what() << std::endl;
	}
}
